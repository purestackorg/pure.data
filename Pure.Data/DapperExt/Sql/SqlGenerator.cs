...
namespace Pure.Data.Sql
{
    public interface ISqlGenerator
    {
        ...
        ISqlCustomGenerator SqlCustomGenerator { get; }
        ...
        string DeleteAll(IClassMapper classMap);
        string Truncate(IClassMapper classMap);
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        public ISqlCustomGenerator SqlCustomGenerator { get; private set; }
        ...
        public virtual string Insert(IClassMapper classMap)
        {
            ...
            if (triggerIdentityColumn.Count > 0)
            {
                ...
                //if (Configuration.Dialect.databaseType == DatabaseType.Oracle || Configuration.Dialect.databaseType == DatabaseType.Firebird)
                //{
                //    sql += Configuration.Dialect.BatchSeperator+ string.Format(" RETURNING {0} INTO {1}IdOutParam", triggerIdentityColumn.Select(p => GetColumnName(classMap, p, false)).First(), Configuration.Dialect.ParameterPrefix);

                //}
                //else if (Configuration.Dialect.databaseType == DatabaseType.PostgreSQL)
                //{
                //    sql += Configuration.Dialect.BatchSeperator + string.Format(" RETURNING {0} AS {1}IdOutParam", triggerIdentityColumn.Select(p => GetColumnName(classMap, p, false)).First(), Configuration.Dialect.ParameterPrefix);

                //}
                //else if (Configuration.Dialect.databaseType == DatabaseType.SqlServer)
                //{
                //    sql += Configuration.Dialect.BatchSeperator + string.Format(" SELECT CAST(SCOPE_IDENTITY()  AS BIGINT) AS {0}IdOutParam", Configuration.Dialect.ParameterPrefix);
        ...
        public virtual string DeleteAll(IClassMapper classMap)
        {
            StringBuilder sql = new StringBuilder(string.Format("DELETE FROM {0}", GetTableName(classMap))); 
            return sql.ToString();
        }
        public virtual string Truncate(IClassMapper classMap)
        {
            StringBuilder sql = new StringBuilder(string.Format("TRUNCATE TABLE {0}", GetTableName(classMap)));
            return sql.ToString();
        }

        public virtual string IdentitySql(IClassMapper classMap)
        {
            return Configuration.Dialect.GetIdentitySql((classMap));
        }
        ...
        {
            IPropertyMap propertyMap = map.Properties.FirstOrDefault(p => p.Name.Equals(propertyName, StringComparison.InvariantCultureIgnoreCase));
            if (propertyMap == null)
            {
