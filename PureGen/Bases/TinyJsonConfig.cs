...
namespace PureGen
{
    ...
    ///  From DynamicJson
    /// </summary>
    public class TinyJsonConfig : DynamicObject
    {
        ...
        #region Config Extend
        /// <summary>
        /// 可以去掉 /* */ 和 //注释，以及去掉空白行，还能去除#注释
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        public static string FormatAndClearMark(string input)
        {
        ...
        public static string LoadText(String filePath, Encoding encoding = null)
        {
            if (encoding == null)
            {
        ...
        private static ConcurrentDictionary<string, WatcherConfig> ConfigPool = new ConcurrentDictionary<string, WatcherConfig>();
        public delegate void UserConfigFileChangedHandler(WatcherConfig watchConfig);
        ...
        public static string FormatAbsFilePath(String filePath)
        {
        ...
        public static bool HasValue(dynamic obj, string key)
        {
            bool defined = obj.IsDefined(key);
            return defined;
        }
        public static T GetValue<T>(dynamic obj, string key, T defaultValue)
        {
            bool defined = obj.IsDefined(key);
            if (defined)
            {
        ...
        public static T GetEnumByValue<T>(string value, T defaulValue)
        {
            try
            {
                if (value == "" || value == null)
        ...
        private static dynamic ToValue(XElement element)
        {
            ...
            switch (type)
            {
                ...
                case JsonType.array:
                    return new TinyJsonConfig(element, type);
                case JsonType.@null:
                default:
                    return null;
            }
        ...
        public TinyJsonConfig()
        {
            xml = new XElement("root", CreateTypeAttr(JsonType.@object));
            jsonType = JsonType.@object;
        }

        private TinyJsonConfig(XElement element, JsonType type)
        {
        ...
        private dynamic DeserializeValue(XElement element, Type elementType)
        {
            ...
            if (value is TinyJsonConfig)
            {
                value = ((TinyJsonConfig)value).Deserialize(elementType);
            }
