...
namespace Pure.Data.Pooling.Impl
{
    public class GenericObjectPool<T> : BaseGenericObjectPool<T>, IObjectPool<T>, IUsageTracking<T>
    {
        ...
        public void ReturnObject(T obj)
        {
            ...
            lock (p)
            {
                if (p.State == PooledObjectState.Idle)
                {
                    return;
                }
                else if (p.State != PooledObjectState.Allocated)
                {
            ...
            if (this._idleObjects.Size > 0)
            {
                ...
                lock (this._evictionLock)
                {
                    ...
                    for (int i = 0, m = this.NumTests; i < m; i++)
                    {
                        ...
                        var evictItems = this.evictionIterator.GetEnumerator();
                        while (evictItems.MoveNext()) //一定要先movenext,不然会有异常
                        {
                            // Console.WriteLine(evictItems.Current.ToString());
                            try
                            {
                                //this.evictionIterator.GetEnumerator().MoveNext();
                                unserTest = evictItems.Current;
                            }
