...
using Pure.Data.Pooling.Collections;
using Pure.Data.Pooling.Impl.Atom;
using Pure.Data.Pooling.Impl.Exceptions; 
namespace Pure.Data.Pooling.Impl
{
    ...
    public class GenericKeyedObjectPool<TKey, TValue> : BaseGenericObjectPool<TValue>, IKeyedObjectPool<TKey, TValue>, IDisposable
    {
        ...
        public GenericKeyedObjectPool(IKeyedPooledObjectFactory<TKey, TValue> factory,
            ...
            : base(genericKeyedObjectPoolConfig, timer)
        {
                ...
                for (int i = 0, m = this.NumTests; i < m; i++)
                {
                    ...
                    else
                    {
                        if (testWhileIdle)
                        {
                            ...
                            {
                                LogHelper.LogErrorInternal(  $"{this.Factory?.GetType().Name} invoke ActivateObject fault. ",e);
                                this.Destory(this._evictionKey, underTest, true);
                                this.DestroyedByBorrowValidationCount.IncrementAndGet();
                            }
                            if (active)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        LogHelper.LogErrorInternal(  $"{this.Factory?.GetType().Name} invoke PassivateObject fault.", e);
                                        this.Destory(this._evictionKey, underTest, true);
                                        this.DestroyedByEvictorCount.IncrementAndGet();
                                    }
                                }
        ...
        public override void Evict()
        {
            ...
            finally
            {
                    ...
                    LogHelper.LogDebugInternal($"pool status: actived {this.NumActive}, destroyed {this.DestroyedCount}, " +
        ...
        private IPooledObject<TValue> Create(TKey key)
        {
            ...
            try
            {
                ...
                LogHelper.LogDebugInternal($"object was created at {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}");
            }
            catch (Exception ex)
            {
                LogHelper.LogErrorInternal(  $"object was creating fault at {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}", ex);
                this._numTotal.DecrementAndGet();
                Interlocked.Decrement(ref objectDeque.createCount);
                throw;
            }
            ...
            try
            {
                ...
                if (isIdle || always)
                {
                    ...
                    try
                    {
                        ...
                        LogHelper.LogDebugInternal($"object was destoried at {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}");
                    }
                    catch (Exception ex)
                    {
                        LogHelper.LogErrorInternal(  $"destory object fault at {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}", ex);
                        throw;
                    }
                    finally
                    {
